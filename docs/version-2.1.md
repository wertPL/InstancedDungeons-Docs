...
The Pro admin GUI also provides a complete checkpoint workflow: list/create checkpoints, set location and order, select the trigger type, bind a gate or targeted button/plate, toggle party teleport, maintain closed gates, and delete checkpoints. Changing the trigger type clears the previous gate/block binding, so **Configure Trigger** must then be used for the newly selected type. The plugin also corrects older checkpoint files containing a button/pressure-plate type left mismatched with their saved material. The **Closed Gates** entry explains that listed gates close immediately without an animation when the checkpoint activates. Targets must have an equal or lower stage order and cannot be the gate that triggered the checkpoint.
